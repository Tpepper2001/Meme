...
export default function AdvancedMemeGenerator() {
  ...
  // THE FIX: Only draw when image is actually loaded
  useEffect(() => {
    ...
    // This is the key: always set src last
    img.src = image;

  }, [image, texts]); // ← This dependency array is perfect now
  ...
  return (
    <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '20px', fontFamily: 'system-ui', background: '#f9fafb', minHeight: '100vh' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px', marginBottom: '32px' }}>
        ...
        >
          ...
          <p style={{ fontSize: '20px', margin: '16px 0' }}>or click to upload</p>
          <input id="fileInput" type="file" accept="image/*" onChange={handleImageUpload} style={{ display: 'none' }} />
        </div>

        <div style={{ background: 'white', borderRadius: '16px', padding: '24px', boxShadow: '0 10px 25px rgba(0,0,0,0.1)' }}>
          <h3 style={{ fontSize: '20px', fontWeight: 'bold', marginBottom: '16px' }}>Templates</h3>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            {memeTemplates.map(t => (
              <button key={t.name} onClick={() => loadTemplate(t.url)} style={{
                padding: '14px', background: '#f1f5f9', border: '1px solid #e2e8f0',
      ...
      {image && (
        <div style={{ display: 'grid', gridTemplateColumns: '380px 1fr', gap: '32px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            {selectedTextObj && (
              <div style={{ background: 'white', padding: '24px', borderRadius: '16px', boxShadow: '0 10px 25px rgba(0,0,0,0.1)', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <input type="text" value={selectedTextObj.content} onChange={e => updateText(selectedText, { content: e.target.value })}
                  style={{ padding: '12px', fontSize: '16px', borderRadius: '8px', border: '2px solid #e2e8f0' }} />

                <input type="range" min="20" max="150" value={selectedTextObj.fontSize}
                  onChange={e => updateText(selectedText, { fontSize: +e.target.value })} />
                <span>Size: {selectedTextObj.fontSize}px</span>
                ...
                <input type="range" min="0" max="15" value={selectedTextObj.strokeWidth}
                  onChange={e => updateText(selectedText, { strokeWidth: +e.target.value })} />
                <span>Outline: {selectedTextObj.strokeWidth}px</span>
                ...
                <span>Rotate: {selectedTextObj.rotation}°</span>

                <label><input type="checkbox" checked={selectedTextObj.uppercase}
                  onChange={e => updateText(selectedText, { uppercase: e.target.checked })} /> UPPERCASE</label>

                <button onClick={() => deleteText(selectedText)} style={{ padding: '12px', background: '#ef4444', color: 'white', border: 'none', borderRadius: '8px' }}>
                  Delete Text
                </button>
              </div>
            )}

            <button onClick={handleDownload} style={{ padding: '20px', background: '#8b5cf6', color: 'white', border: 'none', borderRadius: '16px', fontSize: '20px', fontWeight: 'bold' }}>
              Download Meme
            </button>
          </div>

          <div style={{ background: 'white', borderRadius: '16px', padding: '20px', boxShadow: '0 10px 40px rgba(0,0,0,0.15)' }}>
            <canvas
              ref={canvasRef}
